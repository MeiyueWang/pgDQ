...
data2 <- read.table("corr_vs_pseudotime_of_TF.xls",sep = "\t",stringsAsFactors = F)
GOinfo <- fread("TF_binding/TF_binding.info",sep = "\t",stringsAsFactors = F,header = F)
data.j <- left_join(data2,GOinfo,by=c("V1"="V1"))

TF_sel <- as.character(unlist(read.table("TF_sel.txt",stringsAsFactors = F)))
data.j <- data.j[data.j$V1%in%TF_sel,]
...
GO <- read.table("TF_binding/TF_binding.anno",sep = "\t",stringsAsFactors = F)
cds.data <- read.table("pgDQ_monocle_pseudotime.xls",sep = "\t",header = T,stringsAsFactors = F)
CS_scRNA <- readRDS("CS_mergerep.rds")
...
gsea_zero_GO <-  c()
for (i in c(data.j[c(1:7,9:11),"V1"])) {
  GO_tG <- GO[grep(i,GO$V2,ignore.case = T),"V1"]
  GO_tG_og <- unique(sort(c(orth[orth$cs_A%in%GO_tG,"groups"],orth[orth$cs_B%in%GO_tG,"groups"],orth[orth$cs_D%in%GO_tG,"groups"])))
...
pdf("TF_target_triads_divergence_corr.pdf",width = 5,height = 2)
...
pdf("TF_target_triads_divergence_bg_corr.pdf",width = 5,height = 1.5)
