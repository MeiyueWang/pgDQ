library(ggplot2)
library(cowplot)
df_IS <- read.table("df_IS.txt",sep = "\t",stringsAsFactors = F,header = T)
df_IS$no <- df_IS$no <- c(1:nrow(df_IS))
## Load .edb data
gsea.edb <- read.delim("scRNA_GO_150_1000.Gsea.1712991672305/edb/results.edb",header = FALSE,stringsAsFactors = FALSE)
...
fplot <- function(gene.set,color){
  ...
  ggsave(paste("gsea_GO150_1000_",gsub(":","_",gene.set),".pdf",sep = ""),p,width = 4,height = 4,dpi = 300)
}
fplot("GO:0046658","#A51E26")
fplot("GO:0009834","#CF3F28")
fplot("GO:0004497","#F1612A")
fplot("GO:0016042","#F58227")
fplot("GO:0008643","#F5A822")