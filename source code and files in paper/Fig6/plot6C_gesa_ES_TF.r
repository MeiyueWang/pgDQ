...
## Load .edb data
gsea.edb <- read.delim("scRNA_TF_15_5000.Gsea.1712991622444/edb/results.edb",header = FALSE,stringsAsFactors = FALSE)
...
fplot <- function(gene.set,color){
  ...
  p1 <- ggplot(df.ES,aes(no,ES))+geom_line(linewidth=1,color=color)+
    ...
    guides(color="none")+ylim(-0.2,0.8)
  ...
  ggsave(paste("gsea_TF15_5000_",gsub(":","_",gene.set),".pdf",sep = ""),p,width = 4,height = 4,dpi = 300)
}
fplot("BZIP-A","#A51E26")
fplot("NAC-2D-6","#CF3F28")
fplot("BZIP-D","#F1612A")
fplot("NAM-B1-6D-1","#F58227")
fplot("BZIP-5B-3","#F5A822")