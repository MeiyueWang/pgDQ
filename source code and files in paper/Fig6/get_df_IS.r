dc <- read.table("divGene_out.20240412.xls")
dc <- dc[order(dc$V3,decreasing = F),]
dc$nor <- log(2/(1+dc$V3))
dc$nor_Z <- scale(dc$nor,scale = T,center = T)
dc$no <- c(1:nrow(dc))
orth <- read.table("E:/genome/iwgsc_v1.0/Orthogroups_CS_ABD_1v1.txt",sep = "\t",stringsAsFactors = F,header = T)
...
write.table(dc_out,"df_IS.txt",sep = "\t",col.names = T,row.names = F,quote = F)